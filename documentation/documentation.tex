...
\begin{document}
	...
	In terms of usability I also be consulted with a number of fellow students about design and features to ensure that it is easy for them to use. Utilising usability testing principles from Jakob Nielson \todo{Add to bibliography} I worked with a small number of students and found that sufficent to make usability improvements.
	...
	Event management is a fairly common task and there are a number of solutions out there. A number of event management solutions were quickly appraised to determine fit including Arlo, Grenadine, Facebook Events, Floktu and Eventbrite. \todo{Check event app names \& suitability}
	...
	fast development pace. As well as the large community and environment that comes with it. Using PyPI (a.k.a the Cheese Shop), I have access to over \(113,000\) packages created by the python community that can help me to develop certain features. \todo{git philosiphy? commit fast and commit often}
	...
	Given the variables \(x_{search}\) and \(x_{event}\) are the lattitude in radians, and \(y_{search}\) and \(y_{event}\) are the longitude in radians, we can define the distance between an event and given \textit{search} coordinates in \si{\kilo\metre} as:
